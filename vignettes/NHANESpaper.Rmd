...
We created an R package named phonto, (https://github.com/ainilaha/phonto) that contains some specialized code for both processing the data during our ETL process as well as code for supporting merging tables across cycles (`jointQuery`).
...
![An overview of the ecosystem provided. Panel a) shows the extract transform load (ETL) process. Panel b) Shows examples of tools provide (top) and tools that could easily be used.  Panel c) Notes how these user communities can create reproducible outputs.](images/nhanes1.png)

Figure~\ref{nhanesFig} illustrates the workflow of our system:
...
When analyzing data across cycles there can be substantial issues.  In particular the same table might have different columns, or a variable might have changed its units, or questions and answers may have changed and the population targeted might vary, just to name a few of the issues.  Analysts will want to be know when and where these differences occur and to support those investigations we have included functions to perform quality assessment and control.

The function `qc_var` lets the user specify a set of tables, and a set of variables for examination.  The output is a list of comparisons, such as "description_mismatch", "saslabel_mismatch", or "target_mismatch" and each of those is list of variables for which we have some indication of a mismatch in the metadata across the tables.  
...
that the values (column labels) are inconsistent across cycles. This merely typographic change would have consequences in the modeling if it were not rectified.  While it is usually easy to fix an anomaly once detected finding all anomalies is very challenging. 
...
As well as a table that provides version information and the date the data were downloaded.
