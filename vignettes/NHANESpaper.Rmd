...
When analyzing data for a single cycle, as is most common, the fast access to data is the primary benefit. For such analysis, we suggest using standard R tools to merge datasets, using `nhanesA` to transparently extract necessary data from the database [@ale2024].  Within a cycle, the `SEQN` variable is a unique identifier for each individual, and joins across tables are easily carried out.
