...
An obvious benefit of the system we have built is that users have very rapid access to all the NHANES data at once. This greatly enhances the kinds of analyses that are possible. When analyzing data for a single cycle, as is most common, the fast access to data is the primary benefit. For such analysis, we suggest using standard R tools to merge datasets, using `nhanesA` to transparently extract necessary data from the database [@ale2024].  Within a cycle, the `SEQN` variable is a unique identifier for each individual, and joins across tables are easily carried out.
...
One needs to be careful, however, when combining data across cycles, as there can be substantial inconsistencies. A seemingly innocuous example involves the `DMDEDUC3` variable, which records education level of children and youth, and appears in the `DEMO` table in all cycles. Figure \ref{fig:dmdeduc3} shows the documentation for the `DMDEDUC3` variable from one cycle.  When combining this variable across cycles, it is important to check for consistency in its definition and use across cycles, but doing so manually is time consuming.

We have created an R package called `phonto` [@phonto] that contains a number of R functions to perform various useful tasks taking advantage of the fast database access that would otherwise be too slow to be useful. This includes functions for different database queries as well as for quality assessment (QA) when merging data across cycles and questionnaires. As an example, the function `qc_var()` tests for consistency of a single variable across _all_ tables in NHANES by default. Currently we test for consistency of the SAS Label, the Target (who will be asked the questions), the description (English Text) and whether or not the variable is included in more than one table within a cycle.

The output of a call to the `qc_var()` function for this variable below shows that the SAS label has changed over cycles, albeit in an unimportant way (`Education level` versus `Education Level`). 
```{r qcvar, echo=TRUE}
print(qc_var("DMDEDUC3"), show = "saslabel")
```
Deeper inspection reveals that other typographic changes were made, and at least one of those (see Table \@ref(tab:dmdeduc)) would affect any analysis that involved this variable unless the analyst explicitly ignored case. Analysts will want to know when and where these differences occur before combining data across cycles. By systematically going through all variables in NHANES, we have identified more serious examples where a variable has changed units of measurement across cycles (`LBCBHC`), and a variable name that has been used to represent two entirely different measurements in some cycles (`LBXHCT`). Thankfully, such examples are rare. 
...
The two examples above barely touch upon the many issues that generally arise in the analyis of NHANES data. Apart from the need to properly account for the complex survey design of NHANES and the potential issues in combining data across samples, another important aspect of the NHANES design is the flow of questionnaires. In some cases, a certain response to one question may lead the interviewer to skip several questions. Figure \ref{fig:smoking} shows the documentation for the variable `SMD030` and the preceding variable `SMQ020`.  `SMD030` <!-- appears in `r nrow(xx)` cycles: in the tables `r paste(sprintf("%s%s%s", backtick, xx$TableName, backtick), sep=", ")`, and--> records the age at which a participant started smoking regularly. This is a natural covariate of interest in the study of many diseases including cardiovascular diseases and cancer. In naively using this variable, however, one may neglect an important fact: for many participants for whom the age is recorded as 'Missing', it is actually known from the previous question that the participant has never smoked (leading to this question being skipped). The correct procedure in this case may be to record `SMD030` as right censored at the current age of the participant, which would need to be obtained from a different table.

Delving into these issues in detail is beyond the scope of this article. However, in-depth articles documenting these issues are useful for the wider community of data analysts working on large complex surveys such as NHANES. We plan to work on such documentation in the form of vignettes which we hope to make available at a central website, where we hope to get further contributions from the wider NHANES community. We have made an initial prototype of such a project at <https://epiconnector.github.io/nhanes-docs/>.
