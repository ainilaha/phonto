...
In order to facilitate easy access to the documentation we have replicated all HTML files in the Docker container. The files are organized by cycle and the top level directory is stored as the environment variable `NHANES_TABLE_BASE`.  Using the `browseNHANES` function within the container will access the local files whenever a file name is specified with the `nh_table` argument.  In the future we plan to enable natural language processing (NLP) on these files to enable augmentation of analysis work with new developments in AI.
