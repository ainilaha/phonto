...
When translating one can choose to modify some of the no

### R functions 

We use the `nhanesA` package to bootstrap the ETL process. 
In `phonto` the `addPrimaryKey` function allows the user specifies the table and the columns that will be treated as primary keys. Specifically for the MetaData tables mentioned above, the primary keys are set accordingly.

Users can insert a table into the data base using insert `insertTableDB`
The function `dbInsertNhanesTable` is used to insert NHANES raw and translated tables into
the database.  This is primarily used during the ETL process for building the database.  It
may be useful to advanced users who want to modify how some tables were processed.  But extreme care should be used since that could invalidate any of the reproducibility features of using this system.
